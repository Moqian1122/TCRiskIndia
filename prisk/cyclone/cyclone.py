...
class CycloneExposure:
    ...
    def probability(self) -> float:
        """ The probability of the cyclone event based on the return period"""
        return 1 / self.return_period

    @property
    def poisson_probability(self) -> float:
        """ The probability of the cyclone event based on the return period under the poisson distribution.
...
class CycloneBasinSim:
    ...
    def simulate(self, kernel):
        for exposure in self.entity.cyclone_exposure:
    ...
    def generate_events_set(
            ...
            years:int = 25):
        """
        Generate a set of cyclone events based on the return periods and the number of events
...
class CycloneEntitySim:
    """ The CycloneEntitySim allows the simulation of floods based on
    the exposures of a certain entity """
    def __init__(self, entity, model: str = "poisson", random_seed: Optional[int] = None):
        """
        Initializes the Cyclone class.

        Args:
            entity: The entity object containing relevant data, including cyclone exposure.
            model (str, optional): The statistical model to use. Defaults to "poisson".
            random_seed (Optional[int], optional): The seed for random number generation to ensure reproducibility. Defaults to None.
        """
        self.entity = entity
        self.exposures = entity.cyclone_exposure
        self.model = model
        self.random_seed = random_seed


    def _simulate_poisson(self, time_horizon: float, kernel):
        """ Simulate the cyclone using the Poisson model 
